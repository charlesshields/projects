...
def imageframe(link):
    try:
        app.logger.debug("image_frame({})".format(link))
        fmt = '%Y-%m-%d %H:%M:%S'
        ...
        app.logger.info("session_uid: {} , Image Count: {}".format(current_session_uid,image_count))
        ...
        if image_count > 0:
            ...
            quality_df = quality_df.rename(columns={"images.scene_images.image_uid": "image_uid"})


            scene_df = scene_df.rename(columns={"images.scene_uid": "scene_uid"})
            scene_df = scene_df.rename(columns={"images.task_code": "task_code"})
            scene_df = scene_df.rename(columns={"images.task_name": "task_name"})

            image_urls = pd.DataFrame(scene_df['image_urls'].apply(pd.Series))

            scene_df = scene_df.drop(['image_urls'], axis = 1)
            scene_df = scene_df.drop(['quality_issues'], axis = 1)
            ...
            image_frame = image_frame.rename(columns={"scene_uid": "SceneID"})
            image_frame = image_frame.rename(columns={"task_code": "TaskCode"})
            image_frame = image_frame.rename(columns={"task_name": "TaskName"})
            image_frame = image_frame.rename(columns={"code": "QualityIssueCode"})
            image_frame = image_frame.rename(columns={"value": "QualityIssueValue"})
            image_frame = image_frame.rename(columns={"image_uid": "ImageID"})
            image_frame = image_frame.rename(columns={"session_uid": "SessionID"})
            image_frame = image_frame.rename(columns={"session_start_time": "SessionDateTime"})
            image_frame = image_frame.rename(columns={"session_date": "SessionDate"})
            image_frame = image_frame.rename(columns={"visitor_identifier": "EmailAddress"})
            image_frame = image_frame.rename(columns={"store_number": "StoreNumber"})
            ...
            image_frame = image_frame.drop(['capture_time'], axis = 1) # drops the original capture time from dataframe
            ...
            image_frame.drop_duplicates(keep='first', inplace=True)
            print('connecting to sql engine')
            engine = create_engine('mssql+pyodbc://username:password@server/db?driver=ODBC+Driver+13+for+SQL+Server')
            ...
            app.logger.warn("No Images in session_uid: {} , link: {}".format(current_session_uid,link))

    except ValueError:
        app.logger.error("Failed to decode JSON in {}".format(link))
    except KeyError:
        app.logger.error("No Images in  {}".format(link))
    except Exception as e:
        app.logger.error("Exception in imageframe {}".format(e))  
        app.logger.warn(link)
